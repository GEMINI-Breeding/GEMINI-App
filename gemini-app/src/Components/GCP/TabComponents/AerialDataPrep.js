    ...
    useEffect(() => {
        const fetchDataAndUpdate = async () => {
            if (selectedLocationGCP && selectedPopulationGCP) {
                try {
                    const dates = await fetchData(`${flaskUrl}list_dirs/Raw/${selectedLocationGCP}/${selectedPopulationGCP}`);

                    const orthoCheckPromises = dates.map(async (date) => {
                        const files = await fetchData(`${flaskUrl}list_files/Processed/${selectedLocationGCP}/${selectedPopulationGCP}/${date}/Drone`);
                    ...
                    <div key={"rgb-" + index}>
                        <ListItemButton onClick={() => handleOptionClick(option)}>
                            <Grid container alignItems="center">
                                <Grid item xs={10}>
                                    <ListItemText primary={option.label} primaryTypographyProps={{ fontSize: '1.25rem' }} />
                                </Grid>
                                <Grid item xs={2}>
