...
test("recover-password shows a 'not available' notice and a back link", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(/not available in this deployment/i),
  ).toBeVisible()
  await expect(
...
test("reset-password shows a 'not available' notice and a back link", async ({
  page,
}) => {
