      ...
      {selectedRecords.length > 0 && (
        <div className="space-y-1">
          {selectedRecords.map((r) => (
            <div
              key={r.id}
              className="flex items-center justify-between gap-1 rounded border px-2 py-1 text-xs bg-muted/40"
            >
...
function MultiMetricSelector({
  ...
  const {
    ...
    isLoading,
    isError,
  } = useTraitRecordGeojson(recordId)
  // If the primary record 404s, fall back to the first successful record in the pipeline
  const { firstValid } = useMultiTraitGeojson(
    isError && fallbackRecordIds?.length ? fallbackRecordIds : [],
  )
    ...
    <div className="space-y-1.5">
      ...
      {isError && !geoData && (
        <p className="text-xs text-destructive">
          Could not load metrics — re-run extraction
        </p>
      )}
      {values.length > 0 && (
        <div className="space-y-1">
          {values.map((m, i) => (
            <div key={m} className="flex items-center gap-1.5">
              {/* Axis indicator */}
              <span
                className={`text-[9px] font-bold w-3 flex-shrink-0 ${i === 0 ? "text-primary" : "text-muted-foreground"}`}
              >
      ...
      {values.length > 1 && (
        <p className="text-[10px] text-muted-foreground">
          <span className="font-medium text-primary">L</span> = left axis ·{" "}
          <span className="font-medium">R</span> = right axis (when dual-axis is
          on)
        </p>
      )}
    </div>
...
function FieldSelect({
  ...
  const fields = useMemo(() => {
    ...
    const allKeys = deduplicateKeys(
      geoData.geojson.features.flatMap((feat) =>
  ...
  return (
    <div className="space-y-1.5">
      ...
      {isError && (
        <p className="text-xs text-destructive">
          Could not load fields — re-run extraction
        </p>
      )}
      {!isLoading && !isError && (
        <Select
...
function PipelineSelect({
  ...
  const pipelines = useMemo(() => {
    ...
    const map = new Map<
      string,
      { id: string; name: string; records: string[] }
    >()
    records.forEach((r) => {
      if (!map.has(r.pipeline_id)) {
        map.set(r.pipeline_id, {
          id: r.pipeline_id,
...
function FiltersSection({
  ...
  const availableToAdd = catFields.filter(
    (f) => !Object.keys(filters).includes(f),
  )
  ...
  const totalActive = activeFields.reduce(
    (n, f) => n + (filters[f]?.length ?? 0),
    0,
  )
        ...
        <div className="space-y-2">
          {activeFields.map((field) => {
            ...
            return (
              <div
                key={field}
                className="rounded-md bg-muted/50 border p-2 space-y-1.5"
              >
                <div className="flex items-center justify-between">
  ...
  return (
    <div className="space-y-3">
      ...
      <MetricSelect
        ...
        value={config.metric}
        onChange={(v) => onChange({ ...config, metric: v || null })}
      />
      <div className="space-y-1.5">
        ...
        <Select
          ...
          onValueChange={(v) =>
            onChange({ ...config, aggregation: v as KpiConfig["aggregation"] })
          }
        >
          <SelectTrigger className="h-8 text-xs">
            <SelectValue />
    ...
    <div className="space-y-1.5">
      ...
      {isError && (
        <p className="text-xs text-destructive">
          ...
          {(error as any)?.message ?? "check server connection"}
        </p>
      )}
      ...
      {!isLoading && !isError && datasets.length > 0 && (
        <Select
          value={value || "__none__"}
          onValueChange={(v) => v !== "__none__" && onChange(v)}
        >
          <SelectTrigger className="h-8 text-xs">
            ...
            [
              {
                key: "pipeline-run",
                label: "Pipeline Run",
                desc: "Single extraction date",
              },
              {
                key: "pipeline-avg",
                label: "Pipeline Average",
                desc: "All dates for a pipeline",
              },
              {
                key: "reference",
                label: "Reference Dataset",
                desc: "Uploaded field design / reference traits",
              },
            ] as const
          ).map((opt) => (
            <button
            ...
            onClick={() => {
              onAdd(
                prMetrics.map((m) => ({
                  ...
                  recordId: prRecordId!,
                  metric: m,
                  aggregation: (prAggs[m] ?? "avg") as any,
                  label: prLabel ? `${prLabel} · ${m}` : undefined,
                })),
              )
            }}
          >
            Add{" "}
            {prMetrics.length > 1 ? `${prMetrics.length} sources` : "source"}
          </Button>
          <Button
            size="sm"
            variant="ghost"
    ...
    return (
      <div className="rounded-md border border-dashed p-3 space-y-2 bg-muted/30">
        ...
        <PipelineSelect
          ...
          onChange={(id, records) => {
            setPaId(id)
            setPaPipelineRecordIds(records)
          }}
        />
        <MultiMetricSelector
          ...
          <Button
            ...
            onClick={() => {
              onAdd(
                paMetrics.map((m) => ({
                  id: newId(),
                  type: "pipeline-avg" as const,
                  pipelineId: paId!,
                  metric: m,
                  aggregation: (paAggs[m] ?? "avg") as any,
                  label: paLabel ? `${paLabel} · ${m}` : undefined,
                })),
              )
            }}
          >
            Add{" "}
            {paMetrics.length > 1 ? `${paMetrics.length} sources` : "source"}
          </Button>
          <Button
            size="sm"
            variant="ghost"
  ...
  if (step === "reference") {
    ...
    return (
      <div className="rounded-md border border-dashed p-3 space-y-2 bg-muted/30">
        ...
        />
        <ReferenceMetricSelect
          datasetId={refDatasetId}
          value={refMetric}
          onChange={setRefMetric}
        />
        <div className="space-y-1.5">
          <Label className="text-xs">Aggregation</Label>
...
function MultiSourceGroupBySelector({
  ...
  const firstPipelineSource = sources.find(
    (s) => s.type === "pipeline-run" || s.type === "pipeline-avg",
  )
  const { data: allRecords = [] } = useTraitRecords()

  const recordId = useMemo(() => {
    if (!firstPipelineSource) return null
    if (firstPipelineSource.type === "pipeline-run")
      return firstPipelineSource.recordId
    return (
      allRecords.find(
        (r) => r.pipeline_id === (firstPipelineSource as any).pipelineId,
      )?.id ?? null
    )
  ...
  return (
    <div className="space-y-1.5">
      ...
      {value && (
        <p className="text-[10px] text-muted-foreground">
          X-axis will show unique values of <strong>{value}</strong>. For
          temporal sources, each value becomes its own line.
        </p>
      )}
    </div>
  )
...
function ReferenceFiltersSection({
  ...
  const uniqueByField = useMemo(() => {
    ...
    fields.forEach((f) => {
      const vals = [
        ...new Set(plots.map((p) => String(p[f] ?? "")).filter(Boolean)),
      ].sort()
  ...
  if (Object.keys(uniqueByField).length === 0)
    return (
      <p className="text-[10px] text-muted-foreground">
...
function SourceList({
  ...
  function sourceRecordId(src: DataSource): string | null {
    ...
    if (src.type === "pipeline-avg") {
      return (
        allRecords.find((r) => r.pipeline_id === src.pipelineId)?.id ?? null
      )
    }
    return null
  }
  ...
  const defaultLabelHint = (src: DataSource) => {
    if (src.type === "reference")
      return `${(src as any).datasetId?.slice(0, 6)}… · ${src.metric}`
    if (src.type === "pipeline-run")
      return `${(src as any).recordId?.slice(0, 6)}… · ${src.metric}`
  ...
  return (
    <div className="space-y-2">
      ...
      {sources.length === 0 && !adding && (
        <p className="text-xs text-muted-foreground">
          No sources added yet — add at least one to render the chart.
        </p>
      )}
        ...
        return (
          <div key={src.id} className="rounded-md border bg-card">
            ...
            <div className="flex items-center gap-1.5 p-2.5">
              ...
              <span
                className={`text-[9px] font-bold flex-shrink-0 ${i === 0 ? "text-primary" : "text-muted-foreground"}`}
              >
              ...
              {(recordIdForFilter || src.type === "reference") && (
                ...
                >
                  ...
                  {filterCount > 0 && (
                    <span className="text-[9px] ml-0.5">{filterCount}</span>
                  )}
                </button>
              )}
              <button
            ...
            <div className="grid grid-cols-2 gap-1.5 px-2.5 pb-2.5">
              ...
              {sources.length > 1 && (
                <div className="space-y-1">
                  ...
                  <Select
                    value={src.yAxis ?? "left"}
                    onValueChange={(v) =>
                      updateYAxis(src.id, v as "left" | "right")
                    }
                  >
    ...
    <div className="space-y-3">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="spatial">
              Spatial — compare by category (single run)
            </SelectItem>
            <SelectItem value="temporal">
              Temporal — track change over dates
            </SelectItem>
            <SelectItem value="correlation">
              Correlation — metric vs metric
            </SelectItem>
            <SelectItem value="multi-source">
              Multi-source — compare across pipelines &amp; reference data
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <Select
          ...
          onValueChange={(v) =>
            onChange({ ...config, chartType: v as ChartType })
          }
        >
          <SelectTrigger className="h-8 text-xs">
            <SelectValue />
      ...
      {config.mode === "multi-source" && (
        <>
          ...
          {config.chartType === "bar" &&
            (config.groupByField || (config.sources ?? []).length > 1) && (
              <div className="space-y-1.5">
      ...
      {config.mode === "spatial" && (
        <>
          ...
          <MultiMetricSelector
            ...
            onChange={(metrics) =>
              onChange({ ...config, yAxes: metrics, yAxis: metrics[0] ?? null })
            }
            onAggChange={(metric, agg) =>
              onChange({
                ...config,
                yAxesAggregation: {
          ...
          {(config.yAxes?.length ?? 0) > 1 && (
            <label className="flex items-center gap-2 cursor-pointer">
              <div
                ...
                onClick={() =>
                  onChange({ ...config, dualAxis: !config.dualAxis })
                }
              >
                <div
                  className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white shadow transition-transform ${config.dualAxis ? "translate-x-4" : ""}`}
                />
              </div>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  ...
                  onClick={() =>
                    onChange({
                      ...config,
                      showErrorBand: !config.showErrorBand,
                    })
                  }
                >
                  <div
                    className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white shadow transition-transform ${config.showErrorBand ? "translate-x-4" : ""}`}
                  />
                </div>
              ...
              {config.showErrorBand && (
                <Select
                  ...
                  onValueChange={(v) =>
                    onChange({ ...config, errorBandType: v as any })
                  }
                >
                  <SelectTrigger className="h-7 text-xs">
                    <SelectValue />
...
function TableForm({
  ...
  const activeIds =
    (config.traitRecordIds?.length ?? 0) > 0
      ? config.traitRecordIds
      : config.traitRecordId
        ? [config.traitRecordId]
        : []
      ...
      <div className="space-y-1.5">
        ...
        <Input
          ...
          onChange={(e) =>
            onChange({
              ...config,
              maxRows: parseInt(e.target.value, 10) || 100,
            })
          }
        />
      </div>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={draft.span}
              onValueChange={(v) =>
                setDraft((d) => ({ ...d, span: v as WidgetSpan }))
              }
            >
              <SelectTrigger className="h-8 text-xs">
                <SelectValue />
