...
# GET /files/{id}/metadata-csvs
@router.get("/{id}/metadata-csvs")
def get_metadata_csvs(
    session: SessionDep,
    current_user: CurrentUser,
    id: uuid.UUID,
) -> Any:
    ...
    data_root = Path(get_setting(session=session, key="data_root") or settings.APP_DATA_ROOT)
    storage_dir = data_root / file.storage_path

    def _read_csv(path: Path, label: str) -> dict | None:
        try:
        ...
        # Derive heading_deg from heading_motion (radians) if present
        if "heading_motion" in df.columns and "heading_deg" not in df.columns:
            df["heading_deg"] = df["heading_motion"].apply(
                lambda r: round(math.degrees(float(r)) % 360, 1)
    ...
    def _add(path: Path, label: str) -> None:
        ...
        if resolved in seen or not path.is_file():
            return
        seen.add(resolved)
        entry = _read_csv(path, label)
        if entry:
            results.append(entry)

    # 1. FileUpload.msgs_synced_path (set for Synced Metadata uploads & Farm-ng binaries)
    if file.msgs_synced_path:
        _add(data_root / file.msgs_synced_path, "GPS Sync")

    # 2. Scan Raw storage dir for msgs_synced.csv
    for p in sorted(storage_dir.rglob("msgs_synced.csv")):
        _add(p, "GPS Sync")

    # 3. Scan Raw storage dir for drone_msgs.csv (ArduPilot log data)
    for p in sorted(storage_dir.rglob("drone_msgs.csv")):
        _add(p, "Platform Log")
